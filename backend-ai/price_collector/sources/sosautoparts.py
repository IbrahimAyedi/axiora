...
def search_sosautoparts(make: str, model: str, year: int, part_category: str) -> list:
    """
    Search SOSAutoParts for a given part and vehicle.
    Returns a list of CollectedPart objects.
    """
    query = f"{part_category} {make} {model}"
    
    # Try typical search URLs
    url = "https://sosautoparts.tn/"
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"SOSAutoParts returned status code {response.status_code}")
            return []
        ...
        if not products:
            print("No clear product selectors found for SOSAutoParts. The site may require JavaScript, have different selectors, or the search returned no results.")
        ...
        print(f"Error fetching from SOSAutoParts: {e}")
