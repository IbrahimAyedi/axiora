...
/// Confirmation/edit screen shown after driver license OCR.
/// The user can correct extracted fields before they are committed to the
/// driver profile and the active constat draft.
class PermisResultScreen extends ConsumerStatefulWidget {
  const PermisResultScreen({super.key});

  @override
  ConsumerState<PermisResultScreen> createState() => _PermisResultScreenState();
}

class _PermisResultScreenState extends ConsumerState<PermisResultScreen> {
  ...
  void initState() {
    ...
    _licenseController = TextEditingController(
      text: data?['licenseNumber'] as String? ?? '',
    );
    _cinController = TextEditingController(
      text: data?['nationalId'] as String? ?? '',
    );
    _dobController = TextEditingController(
      text: data?['dateOfBirth'] as String? ?? '',
    );
  ...
  // ---------------------------------------------------------------------------
  // Confidence helpers (same pattern as CarteGriseResultScreen)
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Save confirmed data into driver profile
  // ---------------------------------------------------------------------------

  void _confirm() {
    if (!_formKey.currentState!.validate()) return;

    ref.read(appSessionProvider.notifier).saveConstatDriverDraft(
    ...
    // Return to driver info form in the constat flow
    context.go(RouteNames.driverInfoPath);
  }
      ...
      body: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            AppButton(
              label: 'Confirm and use in driver form',
...
// ---------------------------------------------------------------------------
// Shared sub-widgets (local copies — no shared widget file needed)
// ---------------------------------------------------------------------------
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            ...
            '• Keep the license flat and avoid reflections\n'
